        ...
        <div className="hidden md:flex items-center gap-9">
          {NAV_LINKS.map(link => (
            <Link key={link.href} href={link.href}
              className={cn('text-[15px] font-semibold transition-colors',
                pathname === link.href ? 'text-violet-600' : 'text-gray-900 hover:text-violet-600'
              )}>
              {link.label}
            </Link>
          ))}
        ...
        <div className="md:hidden bg-white border-b border-gray-100 shadow-lg px-6 py-5 space-y-1">
          {NAV_LINKS.map(link => (
            <Link key={link.href} href={link.href} onClick={() => setMobileOpen(false)}
              className="block text-base text-gray-900 hover:text-violet-600 py-3 font-semibold transition-colors">
              {link.label}
            </Link>
          ))}
